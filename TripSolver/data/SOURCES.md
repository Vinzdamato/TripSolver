...
## Estensione del dataset tramite OpenStreetMap
È incluso uno script (`data/fetch_osm_pois.py`) che effettua un'estrazione
live dei POI di una città a scelta tramite l'API Overpass di OpenStreetMap:

```bash
python data/fetch_osm_pois.py --city "Bari, Italy" --out data/poi_osm_raw.csv
```

I dati OpenStreetMap sono distribuiti con licenza **ODbL** (Open Database
License): è consentito l'uso in un progetto accademico citando
"© OpenStreetMap contributors" come richiesto dalla licenza.
...
principali (come fatto per i POI verificati descritti sopra).
